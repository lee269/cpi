...
## `r latest_month` `r latest_year` Food price inflation

#### Data published `r day(release_date)` `r month(release_date, label = TRUE, abbr = FALSE)` `r year(release_date)`

### Aim

The aim of this briefing is to provide a quick overview of the latest inflation figures published by ONS so that Defra staff can have a quick, top-level read out of the key changes soon after the figures are released. It is designed to complement and not replace the existing FCAT "CPI early briefs." It is not designed to be a detailed and insightful analysis of food inflation and it's drivers.
...
ggplot(data) +
  geom_bar(aes(x = cdid, y = value),stat = "identity",  position = "dodge", fill = "red") +
