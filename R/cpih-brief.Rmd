...
## Food price inflation^[By which, we refer to the ONS 'food and non -alcoholic beverage' price index.] summary: `r format(latest_date, "%B %Y")`
...
ggplot(data) +
  ...
  scale_x_date(date_labels = "%b %Y", limits = c(ymd("2020-01-01"), NA)) +
...
### Monthly inflation^[Refers to changes in the price index for food/overall CPIH on a month-on-month basis. These numbers are normally more volatile than year-on-year inflation figures ask they focus on variations within a short timeframe.]
...
x <- map_df(cdids, get_series_metadata, df = metadata) %>% 
  mutate(title = str_to_title(tidy_series_name(title,start = "CPIH: Contribution to all items annual rate: ", end = "#"))) %>% 
  select(cdid, title)
